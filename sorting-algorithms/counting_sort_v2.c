/**
 * 
 * Counting Sort Algorithm Version 2
 * 
 * Description:
 ...
 *  5. Shift the frequencies to the right.
 *  6. Initialize output array.
 *  7. Place the elements in the output array.
 *  8. Copy the elements from the output array to the original array.
 *
 */
...
void counting_sort(int arr[], int n){
    ...
    // Shift the frequencies to the right
    for(int i = n - 1; i > 0; i--){
        count[i] = count[i - 1];
    }
    count[0] = 0; // Set the first element to 0
    ...
    // Copy the elements from the output array to the original array
    for(int i = 0; i < n; i++){
        arr[i] = output[i];
    }
}
